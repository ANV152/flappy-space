...
namespace Flappy
{
	...
	void Falling_Asteroids::DestroyAsteroid( int index ) {
	...
	void Falling_Asteroids::updateAsteroidHealth( int i ) {
		int currentHealth = asteroids.at(i).returnHealth() - 1;
		std::cout << currentHealth << " ";
		if( currentHealth == 0 ) {
			DestroyAsteroid(i);
		}
		else{
			asteroids.at(i).lowerHealth();
		}
	}
